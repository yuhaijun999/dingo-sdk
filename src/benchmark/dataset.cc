...
Dataset::TestEntryPtr BioasqMediumDataset::ParseTestData(const rapidjson::Value& obj) const {
  ...
  if (iter == neighbors_id_map->end()) {
    // LOG(INFO) << fmt::format("id({}) not found in neighbors_id_map", id);
    return nullptr;
  }
  ...
  if (!FLAGS_vector_search_filter.empty()) {
    ...
    if (!already_set_label_name_) {
      int64_t id = item["id"].GetInt64();
      auto iter1 = neighbors_id_map->find(id);
      if (iter1 == neighbors_id_map->end()) {
        LOG(WARNING) << fmt::format("id({}) not found in neighbors_id_map", id);
        return nullptr;
      }

      auto iter2 = scalar_labels_map->find(id);
      if (iter2 == scalar_labels_map->end()) {
        LOG(FATAL) << fmt::format("id({}) not found in scalar_labels_map", id);
        return nullptr;
      }
      label_name_ = iter2->second;
      std::cout << "filter key : labels  value : " << label_name_ << std::endl;
      LOG(INFO) << "filter key : labels  value : " << label_name_;
